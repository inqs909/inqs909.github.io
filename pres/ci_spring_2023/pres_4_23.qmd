...
We use a maximum likelihood approach where the likelihood function measures the probability of observing the data given the parameters.
...
df_colon %$% coxph(Surv(time, status) ~ surg)  %>% #<1>
  summary
```
