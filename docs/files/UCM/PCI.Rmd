...
gg.curve <- ggplot(data=data.frame(x=seq(0,1,length.out = 100),y=sin(pi*seq(0,1,length.out=100))))+geom_line(aes(x=x,y=y),col="red")+geom_line(data=data.frame(x=seq(0,0.5,length.out=50),y=sin(pi*seq(0,0.5,length.out=50))+rnorm(50,0,0.01)),aes(x=x,y=y),col="blue")+theme_bw()+
  ...
  geom_polygon(data = data.frame(x=seq(0.4,0.6,length.out = 50),
                                 y=Kh.vec.t(seq(0.4,0.6,length.out = 50),0.5,0.1)/20),
               aes(x=x,y=y))+
  guides(fill=FALSE) + 
  geom_point(data.frame(x= xxx , y = sin(pi*xxx) + rnorm(100, sd = 0.1)), mapping=aes(x=x, y=y), size = 3, col = "brown")
...
- `r Citet(Bib,"wuKERNELSMOOTHINGVARYING2000")`

- `r Citet(Bib,"fanProfileLikelihoodInferences2005")`

- `r Citet(Bib,"fanAnalysisLongitudinalData2007a")`
...
$$
    SE\{\hat f(t)\} = \sqrt{\frac{1}{B}\sum^B_{b=1}\{\hat f_b(t)-\bar f(t)\}^2},
$$
...
- `r Citet(Bib,"xueEmpiricalLikelihoodVarying2007")`

- `r Citet(Bib,"fanSemiparametricEstimationCovariance2008")`
